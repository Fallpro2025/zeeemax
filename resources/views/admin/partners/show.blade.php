...
@section('title', 'Partenaire: ' . $partner->nom . ' - ZEEEMAX Admin')
@section('page-title', 'Détails du Partenaire')
@section('page-description', 'Consultez les informations du partenaire')
    ...
    <div class="flex items-center justify-between mb-8 animate-fade-in">
        <div>
            <div class="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400 mb-2">
                ...
                <span class="text-blue-600">{{ $partner->nom }}</span>
            </div>
            <div class="flex items-center space-x-3">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $partner->nom }}</h1>
                @if($partner->actif)
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="flex items-start space-x-6">
                    ...
                    @else
                        <div class="flex-shrink-0 w-32 h-32 bg-gray-200 dark:bg-gray-700 rounded-lg flex items-center justify-center">
                            <span class="text-2xl font-bold text-gray-400">{{ substr($partner->nom, 0, 2) }}</span>
                        </div>
                    @endif
                    
                    <div class="flex-1">
                        <div class="flex items-center space-x-4 mb-4">
                            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $partner->nom }}</h2>
                        </div>
                        
                        @if($partner->description)
                            <div class="prose dark:prose-invert max-w-none">
                                <p class="text-gray-700 dark:text-gray-300 leading-relaxed">{{ $partner->description }}</p>
                            </div>
                        @endif
                        
                        @if($partner->site_web)
                            <div class="mt-6">
                                <a href="{{ $partner->site_web }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-6">
            ...
            <div class="glass dark:glass-dark rounded-2xl p-6 animate-slide-up" style="animation-delay: 0.1s;">
                ...
                <div class="space-y-3">
                    <form action="{{ route('admin.partners.destroy', $partner) }}" method="POST" class="w-full">
                        @csrf
                        @method('DELETE')
                        <button type="submit" 
            ...
            <div class="glass dark:glass-dark rounded-2xl p-6 animate-slide-up" style="animation-delay: 0.2s;">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm font-medium text-gray-900 dark:text-white">#{{ $partner->id }}</span>
                    </div>
                    
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600 dark:text-gray-400">Statut</span>
                        <span class="text-sm font-medium {{ $partner->actif ? 'text-green-600' : 'text-red-600' }}">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $partner->ordre }}</span>
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $partner->created_at->format('d/m/Y') }}</span>
                    </div>
                    
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600 dark:text-gray-400">Modifié le</span>
                        <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $partner->updated_at->format('d/m/Y') }}</span>
                    </div>
                </div>
            </div>
        </div>
