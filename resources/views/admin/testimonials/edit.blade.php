...
<div>
    ...
    <div class="glass dark:glass-dark rounded-2xl overflow-hidden animate-slide-up">
        <form action="{{ route('admin.testimonials.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 p-8">
            @csrf
            @method('PUT')
        ...
        <div class="px-8 pb-8 border-t border-gray-200 dark:border-gray-700 pt-6">
            <form action="{{ route('admin.testimonials.destroy', $testimonial->id) }}" method="POST" id="delete-form-{{ $testimonial->id }}">
                @csrf
                @method('DELETE')
            </form>
            <button type="button" 
                    onclick="confirmDeleteTestimonial({{ $testimonial->id }}, '{{ $testimonial->nom_client }}')"
