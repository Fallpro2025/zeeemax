    ...
    <div class="flex items-center justify-between mb-8 animate-fade-in">
        ...
        <div class="glass dark:glass-dark rounded-2xl overflow-hidden animate-slide-up" style="animation-delay: 0.2s;">
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                <form id="filterForm" method="GET" action="{{ route('admin.testimonials.index') }}" class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <select name="statut" id="statutFilter" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                            ...
                            <option value="featured" {{ $statut === 'featured' ? 'selected' : '' }}>Mis en avant</option>
                        </select>
                        <!-- Sort -->
                        <select name="sort_by" id="sortBy" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                            ...
                            <option value="nom_client" {{ $sortBy === 'nom_client' ? 'selected' : '' }}>Nom client</option>
                            <option value="note" {{ $sortBy === 'note' ? 'selected' : '' }}>Note</option>
                        ...
                        <a href="{{ route('admin.testimonials.index') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600 transition-colors">
                            Réinitialiser
                        </a>
                        @endif
                    </div>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <div class="flex-shrink-0 h-12 w-12">
                                        @if($testimonial->avatar_url)
                                            <img class="h-12 w-12 rounded-full object-cover" 
                                                 src="{{ str_starts_with($testimonial->avatar_url, 'http') ? $testimonial->avatar_url : asset($testimonial->avatar_url) }}" 
                                                 alt="{{ $testimonial->nom_client }}"
                                                 onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name={{ urlencode($testimonial->nom_client) }}&background=blue&color=fff&size=48'">
                                        @else
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900 dark:text-white max-w-xs truncate">
                                    "{{ Str::limit($testimonial->contenu, 60) }}"
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <form method="POST" action="{{ route('admin.testimonials.destroy', $testimonial) }}" class="delete-form-{{ $testimonial->id }}">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                    <button type="button" onclick="confirmDelete({{ $testimonial->id }}, '{{ $testimonial->nom_client }}', 'témoignage')" class="text-red-600 hover:text-red-900 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<script>
    ...
    async function confirmDelete(id, name, type) {
        const confirmed = await window.adminConfirm.confirm(
            `Êtes-vous sûr de vouloir supprimer ce ${type} de "${name}" ? Cette action est irréversible.`,
            {
